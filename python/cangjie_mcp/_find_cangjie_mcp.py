...
class CanjieMcpNotFound(FileNotFoundError): ...
...
def find_cangjie_mcp_bin() -> str:
    ...
    targets = [
        ...
        (
            # On Windows, with module path `<prefix>/Lib/site-packages/cangjie_mcp`
            _join(
                _matching_parents(
                    _module_path(), "Lib/site-packages/cangjie_mcp"
                ),
                "Scripts",
            )
            if sys.platform == "win32"
            # On Unix, with module path `<prefix>/lib/python3.13/site-packages/cangjie_mcp`
            else _join(
                _matching_parents(
                    _module_path(), "lib/python*/site-packages/cangjie_mcp"
                ),
                "bin",
            )
        ),
        # Adjacent to the package root, e.g., from `pip install --target`
        # with module path `<target>/cangjie_mcp`
        _join(_matching_parents(_module_path(), "cangjie_mcp"), "bin"),
    ...
    for target in targets:
        ...
        if target in seen:
            continue
        seen.append(target)
        path = os.path.join(target, cangjie_mcp_exe)
...
def _user_scheme() -> str:
    if sys.version_info >= (3, 10):
        ...
        user_scheme = "nt_user"
    elif sys.platform == "darwin" and sys._framework:  # type: ignore[attr-defined]
